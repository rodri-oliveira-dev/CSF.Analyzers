...
public sealed class Arc001RequireExplicitAuthorizationOnHttpEndpointsAnalyzerTests
{
    [Fact]
...
[*.cs]
dotnet_diagnostic.ARC001.ignored_namespaces = ["Sample.Public"]
""";
...
""";
    ...
    [Fact]
    public async Task Malformed_allowed_routes_configuration_is_ignored()
    {
        const string editorConfig = """
root = true

[*.cs]
dotnet_diagnostic.ARC001.allowed_routes = ["/internal/status",
""";
...
public sealed class StatusController : ControllerBase
{
    [{|#0:HttpGet|}("/internal/status")]
    public void Get() { }
}
""";

        await VerifyMvcAsync(source, editorConfig, Expected(0, "StatusController.Get"));
    }

    [Fact]
    public async Task Malformed_allowed_methods_configuration_is_ignored()
    {
        const string editorConfig = """
root = true
...
public sealed class DiagnosticsController : ControllerBase
{
    [{|#0:HttpGet|}("diagnostics/ping")]
    public void Ping() { }
}
""";

        await VerifyMvcAsync(source, editorConfig, Expected(0, "DiagnosticsController.Ping"));
    }

    [Fact]
    public async Task Malformed_ignored_namespaces_configuration_is_ignored()
    {
        const string editorConfig = """
root = true

[*.cs]
dotnet_diagnostic.ARC001.ignored_namespaces = [123]
""";
...
""";
    ...
    [Fact]
    public async Task Empty_or_whitespace_entries_do_not_allow_routes_methods_or_namespaces()
    {
        const string editorConfig = """
root = true

[*.cs]
dotnet_diagnostic.ARC001.allowed_routes = ["", "   "]
dotnet_diagnostic.ARC001.allowed_methods = ["", "   "]
dotnet_diagnostic.ARC001.ignored_namespaces = ["", "   "]
""";
...
""";
    ...
    {
        return Verifier<Arc001RequireExplicitAuthorizationOnHttpEndpointsAnalyzer>.VerifyAnalyzerAsync(source, editorConfig, expected);
    }

    private static DiagnosticResult Expected(int location, string endpoint)
    {
        return Verifier<Arc001RequireExplicitAuthorizationOnHttpEndpointsAnalyzer>.Diagnostic("ARC001")
