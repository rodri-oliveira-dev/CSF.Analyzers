# ARCH007: Detecte concatenação de string em loop
...
### Interpolacao com auto-referência
...
- A regra não tenta estimar o numero de iterações em tempo de compilação. Se um loop executa poucas vezes em prática, ainda pode ser reportado (exceto em casos com condição constante `false`).
...
2. **Atribuicao com auto-referência**: `s = s + expr` (inclui variacoes como `s = s + a + b`)
