...
```xml
<PackageReference Include="CSF.Analyzers" PrivateAssets="all" />
```

Depois, escolha os pacotes necessários:

```xml
<PackageReference Include="CSF.Analyzers.Reliability" PrivateAssets="all" />
<PackageReference Include="CSF.Analyzers.Architecture" PrivateAssets="all" />
<PackageReference Include="CSF.Analyzers.Testing" PrivateAssets="all" />
```

Não há metapacote `CSF.Analyzers` na v2 inicial.

## Pacotes de destino

| Pacote | IDs ativos |
| ------ | ---------- |
| `CSF.Analyzers.Reliability` | `REL001`, `REL002`, `REL003`, `REL004`, `REL005`, `REL006` |
| `CSF.Analyzers.Architecture` | `ARC001`, `ARC002`, `ARC003`, `ARC004`, `ARC005`, `ARC006` |
| `CSF.Analyzers.Testing` | `TST001`, `TST002` |
...
| ----- | --------- | ---------------- |
| `REL005` | `CSF.Analyzers.Reliability` | habilitada, warning |
| `REL006` | `CSF.Analyzers.Reliability` | habilitada, warning |
| `ARC006` | `CSF.Analyzers.Architecture` | opt-in, info |
...
Atualize etapas que empacotavam ou validavam `CSF.Analyzers` como pacote único. A v2 deve restaurar, compilar, testar e empacotar os três projetos ativos:
...
1. Remova `CSF.Analyzers` da linha 1.x.
