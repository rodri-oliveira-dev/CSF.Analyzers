...
Nota historica: quando está revisão foi escrita, a versão oficial do pacote era `1.1.0` e ficava em `src/CSF.Analyzers.Core/CSF.Analyzers.Core.csproj` no atributo `<VersionPrefix>`. O fluxo atual usa GitVersion, configurado em `GitVersion.yml`, como fonte da versão publicada.

SDK .NET fixado: `10.0.203`, identificado em `global.json` com `rollForward` para `latestFeature`.

Target frameworks identificados:

- `src/CSF.Analyzers.Core`: `netstandard2.0`
- `src/CSF.Analyzers.SampleApp`: `net10.0`
- `tests/CSF.Analyzers.Tests`: `net10.0`

## Stack principal identificada
- Projeto de analyzers Roslyn reutilizáveis para .NET.
- Pacote NuGet `CSF.Analyzers`, empacotado a partir de `src/CSF.Analyzers.Core`.
...
- `docs/adoption.md`
- `docs/release.md`
- `src/CSF.Analyzers.Core/CSF.Analyzers.Core.csproj`
- `src/CSF.Analyzers.SampleApp/CSF.Analyzers.SampleApp.csproj`
- `tests/CSF.Analyzers.Tests/CSF.Analyzers.Tests.csproj`
- `src/CSF.Analyzers.SampleApp/README.md`
...
- arquivo: `.agents/skills/dotnet-service-change/SKILL.md`
  ...
  impacto: poderia criar ambiguidade entre não implementar autenticação de uma API e permitir alterações legítimas em regras de analyzer de segurança.
...
- arquivo: `.agents/skills/dotnet-service-change/SKILL.md`
  ...
  decisão tomada: remover a referência legada e substituir o fluxo mínimo por dados reais de stack e versão do `CSF.Analyzers`.
...
- `dotnet restore ./CSF.Analyzers.slnx`: sucesso.
- `dotnet build ./CSF.Analyzers.slnx --configuration Release --no-restore`: sucesso, com warnings esperados do SampleApp e exemplos inválidos.
- `dotnet test ./CSF.Analyzers.slnx --configuration Release --no-build -m:1`: sucesso, 432 testes aprovados.
